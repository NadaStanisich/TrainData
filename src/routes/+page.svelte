<script lang="ts">
    ...
    import { TrainDeparture } from '$lib/traindeparture';
    ...
    async function fetchData() {
      try {
        ...
        if (selectedStation) {
          ...
          const departureResponse = await fetch(`https://ptvapiwrapper.azurewebsites.net/trains/get-departures/${selectedStation.stop_id}`, {
            method: 'GET',
            headers: {
              'accept': 'application/json',
              'X-Api-Key': 'pDDuZJyKSfzVU1zRriSw4vWvzpRAoGIAtw0osQPqkwn9MOIpOVpGTeEoMM94jXZw',
          ...
          if (departureResponse.ok) {
            ...
            departureData = departureDataResponse.map((departure: any) =>
              new TrainDeparture(
                departure.stop_id,
                departure.route_id,
                departure.run_id,
            ...
            throw new Error(`Failed to fetch departure data: ${departureResponse.statusText}`);
          }
          ...
          if (directionResponse.ok) {
            ...
            directionData = directionDataResponse.map((direction: any) =>
              new TrainDirections(direction.direction_id, direction.route_direction_description, direction.direction_name, direction.route_id, direction.route_type)
            );
            console.log(directionData);
          } else {
            throw new Error(`Failed to fetch direction data: ${directionResponse.statusText}`);
          }
        }
      } catch (error) {
        console.error(error);
    ...
    async function fetchNextTrains() {
      try {
        if (selectedStation) {
          ...
          const departuresResponse = await fetch(`https://ptvapiwrapper.azurewebsites.net/trains/get-departures/${selectedStation.stop_id}`, {
            method: 'GET',
            headers: {
              'accept': 'application/json',
              'X-Api-Key': 'pDDuZJyKSfzVU1zRriSw4vWvzpRAoGIAtw0osQPqkwn9MOIpOVpGTeEoMM94jXZw',
          ...
          if (departuresResponse.ok) {
            ...
            departureData = departuresData.map((departure: any) =>
              new TrainDeparture(
                departure.stop_id,
                departure.route_id,
                departure.run_id,
            ...
            throw new Error(`Failed to fetch departures data: ${departuresResponse.statusText}`);
          }
        }
      } catch (error) {
        console.error(error);
  ...
  <main>
    ...
    <div class="container mx-auto p-4">
      ...
      {#if stopData.length > 0}
        {#each stopData as stop (stop.stop_id)}
        <div>
            <p>Stop Name: {stop.stop_name}   Stop Suburb: {stop.stop_suburb} Route Type: {stop.route_type} Latitude: {stop.stop_latitude} Longitude: {stop.stop_longitude}</p>
        </div>
        {/each}
        {:else}
      ...
      {#if departureData.length > 0}
        {#each departureData as departure (departure.stop_id)}
            <div>
            <p>Stop ID: {departure.stop_id} Route ID: {departure.route_id} Run ID: {departure.run_id} Direction ID: {departure.direction_id} Scheduled Departure: {departure.scheduled_departure_utc} Estimated Departure: {departure.estimated_departure_utc} Platform Number: {departure.platform_number}</p>
            </div>
        {/each}
      {:else}
     ...
     {/if}
      ...
      <div>
        <p>Direction ID: {direction.direction_id} Route Direction Description: {direction.route_direction_description} Direction Name: {direction.direction_name} Route ID: {direction.route_id} Route Type: {direction.route_type}</p>
      </div>
      {/each}
      {:else}
